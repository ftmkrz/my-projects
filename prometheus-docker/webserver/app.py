...
def number_of_contact():
    ...
    '''
    result = db.session.execute(query)
    count = ''.join(i for i in str(list(result)) if i.isdigit())
    return '''# TYPE number_of_contact gauge\n# HELP test metric\nnumber_of_contact{label="phonebook_application"} %d''' % int(count)
...
def metrics():
    response = make_response(number_of_contact(), 200)
